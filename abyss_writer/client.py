...
logger = logging.getLogger("AbyssWriter.Client")
...
def optimize_prompt(text: str) -> str:
    ...
    # 1. Clean consecutive duplicate/empty lines and strip leading/trailing whitespace
    lines = text.split("\n")
    cleaned_lines = []
    prev_line = None
    for line in lines:
        ...
        if not stripped and not prev_line:
            continue
        cleaned_lines.append(line)  # Keep original indentation/spacing for non-empty lines
        prev_line = stripped
        
    text = "\n".join(cleaned_lines).strip()
    
    # 2. De-duplicate section blocks if any got appended multiple times
    text = "\n" + text
    sections = re.split(r'(\n\[[^\]]+\]\n|\n===+[\s\S]+?===+\n)', text)
    if len(sections) > 1:
        seen_sections = set()
        ...
        while i < len(sections):
            sec = sections[i]
            # If it's a section header, check if the content that follows is a duplicate
            if i + 1 < len(sections) and (sec.startswith("\n[") or sec.startswith("\n=")):
                header = sec
                content = sections[i+1]
                key = (header.strip(), content.strip())
    ...
    # 3. Trim excessive spaces (e.g., more than 3 consecutive empty lines to exactly 1)
    text = re.sub(r'\n{3,}', '\n\n', text)
...
class LlamaAPIClient:
    """
    Abyss Writer API Client for Llama.cpp Server.
    """
    def __init__(
        self,
