...
# 1. Read dry_run_temp ui.py
src = "d:/DeepScribe/dry_run_temp/abyss_writer/ui.py"
dst = "d:/DeepScribe/temp_ui.py"

with open(src, "r", encoding="utf-8") as f:
    code = f.read()

# 2. Define all search patterns and replacements from patch_ui.py
...
def do_sub(pattern, replacement, text, name):
    res, count = re.subn(pattern, replacement, text, flags=re.DOTALL)
...
code = do_sub(insert_helper_pattern, r"\1\n" + new_helpers, code, "4. insert_helpers")
...
# 5. Write the patched content to temp_ui.py
with open(dst, "w", encoding="utf-8") as f:
    f.write(code)

# 6. Write replacement log to file
with open("d:/DeepScribe/patch_log.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(log) + "\n")
