...
from novel_translator.services.gnuboard_db import get_boards, search_posts, merge_posts_content, register_post_to_gnuboard
...
def on_merge_to_translator(bo_table, selected_ids_str):
    ...
    if not selected_ids_str or not selected_ids_str.strip():
        return "❌ 오류: 선택된 게시글이 없습니다. 테이블에서 체크박스를 선택해 주세요.", "", "", gr.update(interactive=False), gr.update(interactive=False)
        
    wr_ids = [x.strip() for x in selected_ids_str.split(",") if x.strip()]
    
    merged_text = merge_posts_content(bo_table, wr_ids)
    if not merged_text:
        return "❌ 오류: 게시글 본문을 가져오지 못했습니다.", "", "", gr.update(interactive=False), gr.update(interactive=False)
    ...
    return info_msg, normalized_text, normalized_text, gr.update(interactive=True), gr.update(interactive=True)
...
def on_translation_complete(translated_text, file_obj):
...
def load_pending_word_details(word):
    """Load details of a selected pending word."""
    if not word:
        return "", "", "", "", "", ""
    try:
        conn = sqlite3.connect(orchestrator.onomatopoeia_db_path)
        ...
        logger.error(f"Error loading pending word details: {e}")
    return "", "", "", "", "", ""


def approve_pending_word(word, translation, notes, ex_src, ex_wrong, ex_correct):
    """Approve a word: save to CSV, update DB status, and reload AC tree."""
    if not word or not translation:
        return "❌ 오류: 단어와 추천 번역은 필수 입력 항목입니다.", gr.update(), [], gr.update()

    # Append to CSV
    try:
        file_path = orchestrator.onomatopoeia_csv
        ...
        # Update SQLite DB status
        db = OnomatopoeiaDB(orchestrator.onomatopoeia_db_path)
        db.set_status(word, "approved")

        if already_exists:
            # Reload dictionary in memory
            orchestrator.reload_onomatopoeia()
            pending = db.get_pending_review()
            pending_choices = [item["word"] for item in pending]
            dict_data = load_current_dictionary()
            registered_choices = [r[0].strip() for r in dict_data if r and r[0].strip()]
            reg_dropdown_update = gr.update(choices=registered_choices, value=registered_choices[0] if registered_choices else None)
            return f"ℹ️ '{word}'는 이미 사전에 등록되어 있습니다. (중복 등록 방지)", gr.update(choices=pending_choices, value=pending_choices[0] if pending_choices else None), dict_data, reg_dropdown_update
        ...
        # Get updated CSV list
        dict_data = load_current_dictionary()
        registered_choices = [r[0].strip() for r in dict_data if r and r[0].strip()]
        reg_dropdown_update = gr.update(choices=registered_choices, value=registered_choices[0] if registered_choices else None)

        return f"✅ '{word}' 승인 완료! 사전에 등록 및 반영되었습니다.", gr.update(choices=pending_choices, value=pending_choices[0] if pending_choices else None), dict_data, reg_dropdown_update

    except Exception as e:
        logger.error(f"Error approving word '{word}': {e}")
        return f"❌ 승인 처리 오류: {e}", gr.update(), [], gr.update()


def reject_pending_word(word):
    ...
    try:
        ...
        return f"❌ '{word}' 반려 완료.", gr.update(choices=choices, value=choices[0] if choices else None)
    except Exception as e:
        logger.error(f"Error rejecting word '{word}': {e}")
        return f"❌ 반려 처리 오류: {e}", gr.update()


def approve_all_pending_words():
    """Approve all pending review words at once: save all to CSV, update DB status, and reload dictionary."""
    try:
        db = OnomatopoeiaDB(orchestrator.onomatopoeia_db_path)
        pending = db.get_pending_review()

        if not pending:
            return "ℹ️ 승인할 대기 단어가 없습니다.", gr.update(), [], gr.update()
        ...
        # Load existing terms in the CSV first to avoid duplicates
        existing_words = set()
        if os.path.exists(file_path):
            try:
                content = None
                ...
                logger.error(f"Error checking duplicates in CSV: {e}")

        # Process each pending word
        approved_count = 0
        duplicate_count = 0

        # Open file once to append all approved words
        with open(file_path, "a", encoding="utf-8-sig", newline="") as f:
            writer = csv.writer(f)
            for row in pending:
                word = row["word"]
                translation = row["suggested_translation"] or ""
                notes = row["notes"] or ""
                ex_src = row["example_source"] or ""
                ex_wrong = row["example_wrong"] or ""
                ex_correct = row["example_correct"] or ""
        ...
        # Get updated pending list (should be empty or remaining if any errors)
        updated_pending = db.get_pending_review()
        pending_choices = [item["word"] for item in updated_pending]
        ...
        msg = f"✨ 총 {approved_count}개 단어가 일괄 승인 완료 및 사전에 등록되었습니다."
        if duplicate_count > 0:
            msg += f" (중복 항목 {duplicate_count}개 자동 처리됨)"
        return msg, gr.update(choices=pending_choices, value=pending_choices[0] if pending_choices else None), dict_data, reg_dropdown_update

    except Exception as e:
        logger.error(f"Error in approve_all_pending_words: {e}")
        return f"❌ 일괄 승인 처리 오류: {e}", gr.update(), [], gr.update()


def get_registered_choices():
    """Fetch words that are registered in the CSV dictionary."""
    file_path = orchestrator.onomatopoeia_csv
    if not os.path.exists(file_path):
    ...
    try:
        ...
        logger.error(f"Error fetching registered choices: {e}")
    
    return gr.update(choices=choices, value=choices[0] if choices else None)


def load_registered_word_details(word):
    """Load details of a selected registered word from the CSV."""
    if not word:
        return "", "", "", "", "", ""
    ...
    try:
        ...
        logger.error(f"Error loading registered word details: {e}")
    return "", "", "", "", "", ""


def update_registered_word(word, translation, notes, ex_src, ex_wrong, ex_correct):
    """Update a registered word: modify in CSV and reload AC tree."""
    if not word or not translation:
        return "❌ 오류: 단어와 추천 번역은 필수 입력 항목입니다.", gr.update(), []

    file_path = orchestrator.onomatopoeia_csv
    if not os.path.exists(file_path):
        return "❌ 오류: 사전 파일이 존재하지 않습니다.", gr.update(), []

    try:
        rows = []
        header = ["source", "target", "category", "notes", "example_source", "example_wrong", "example_correct"]
        ...
        if content:
            ...
            for row_num, row in enumerate(reader, 1):
                if not row:
                    continue
                if row_num == 1:
                    if row[0].lower() in ("source", "원문", "원어", "소스"):
                        header = row
                        continue
                rows.append(row)
        ...
        for idx, row in enumerate(rows):
            if row and row[0].strip() == word.strip():
                rows[idx] = [
                    word.strip(),
                    translation.strip(),
                    row[2].strip() if len(row) > 2 else "의성어",
                    notes.strip(),
                    ex_src.strip(),
                    ex_wrong.strip(),
                    ex_correct.strip()
        ...
        choices = [r[0].strip() for r in rows if r and r[0].strip()]
        dict_data = load_current_dictionary()

        return f"✅ '{word}' 수정 완료! 사전에 반영되었습니다.", gr.update(choices=choices, value=word), dict_data

    except Exception as e:
        logger.error(f"Error updating registered word '{word}': {e}")
        return f"❌ 수정 처리 오류: {e}", gr.update(), []


def delete_registered_word(word):
    """Delete a registered word: remove from CSV and reload AC tree."""
    if not word:
        return "❌ 오류: 선택된 단어가 없습니다.", gr.update(), []
    ...
    try:
        ...
        return f"❌ '{word}' 삭제 완료! 사전에서 삭제되었습니다.", gr.update(choices=choices, value=choices[0] if choices else None), dict_data

    except Exception as e:
        logger.error(f"Error deleting registered word '{word}': {e}")
        return f"❌ 삭제 처리 오류: {e}", gr.update(), []


def select_word_from_dict_view(evt: gr.SelectData):
    """When a row is clicked in dict_view, load it into the Edit/Delete tab."""
    row_idx = evt.index[0]
    dict_data = load_current_dictionary()
    if 0 <= row_idx < len(dict_data):
        row = dict_data[row_idx]
        word = row[0]
        details = load_registered_word_details(word)
        return (gr.update(selected="tab_registered"), word) + details
    return (gr.update(), None, "", "", "", "", "", "")


def load_current_dictionary():
    """Load entries from the CSV dictionary for the dataframe view."""
    file_path = orchestrator.onomatopoeia_csv
    if not os.path.exists(file_path):
        return []
    ...
    try:
        ...
        if content:
            ...
            for row_num, row in enumerate(reader, 1):
                ...
                if row_num == 1 and row[0].lower() in ("source", "원문", "원어", "소스"):
                    continue
                word_clean = row[0].strip()
                if word_clean in seen_words:
                    continue
                seen_words.add(word_clean)
                # Append row to table data
                data.append(row)
    except Exception as e:
        logger.error(f"Error loading current dictionary: {e}")
    return data
...
def create_ui() -> gr.Blocks:
    ...
    with gr.Blocks(
                    ...
                    with gr.Column(scale=2):
                        with gr.Tabs() as edit_tabs:
                            with gr.Tab("📥 신규 검수 대기", id="tab_pending"):
                                ...
                                with gr.Row():
                                    btn_approve = gr.Button("✅ 승인 (Approve)", variant="primary")
                                    btn_reject = gr.Button("❌ 반려 (Reject)", variant="stop")
                                    btn_approve_all = gr.Button("✨ 일괄 승인 (Approve All)", variant="secondary")
                            ...
                            with gr.Tab("📖 등록된 사전 편집/삭제", id="tab_registered"):
                                ...
                                with gr.Row():
                                    btn_update = gr.Button("💾 수정 저장 (Save)", variant="primary")
                                    btn_delete = gr.Button("🗑️ 삭제 (Delete)", variant="stop")
                                    
                        action_status = gr.Markdown("")
                        
                    # Right column: Current Dictionary View
                    with gr.Column(scale=3):
                        gr.Markdown("#### 📖 현재 등록된 의성어 사전 목록 (japanese_erotic_onomatopoeia.csv)")
                        btn_refresh_dict = gr.Button("🔄 사전 새로고침", size="sm")
                        dict_view = gr.Dataframe(
                            headers=["source", "target", "category", "notes", "example_source", "example_wrong", "example_correct"],
        ...
        btn_approve.click(
            ...
            outputs=[action_status, pending_word_dropdown, dict_view, registered_word_dropdown],
        )
        ...
        # 5. Refresh current dictionary dataframe view
        btn_refresh_dict.click(
        ...
        registered_word_dropdown.change(
            ...
            inputs=[registered_word_dropdown],
            outputs=[
                reg_word_input,
