...
SUPPORTED_TARGdef get_system_prompt(
    source_lang: str,
    target_lang: str,
    glossary_text: str = "",
    context_text: str = "",
...
9. **의성어/의태어**: {src['ko']} 특유 of 의성어·의태어는 {tgt['ko']}에서 가장 적절한 표현으로 변환한다.
...
---"""
    ...
    return prompt�`)로 표기하고, 인물의 마음속 생각이나 **내면 독백**은 반드시 작은따옴표(`‘’`)로 표기하십시오. 원문의 괄호 부호(예: 「」, 『』 등)는 한국어 소설 관행에 맞춰 대화는 큰따옴표(`“”`), 생각/독백은 작은따옴표(`‘’`)로 철저히 변환하여 적용합니다.
